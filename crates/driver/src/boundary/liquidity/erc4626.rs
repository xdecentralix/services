use {
    ...
    solver::liquidity::{erc4626::{Erc4626LiquiditySource, Erc4626Order}},
    solver::liquidity_collector::{BackgroundInitLiquiditySource, LiquidityCollecting},
    std::time::Duration,
};

/// Builds the ERC4626 liquidity collector if enabled via configs/<chain>/erc4626.toml.
pub async fn maybe_collector(eth: &Ethereum) -> AnyResult<Vec<Box<dyn LiquidityCollecting>>> {
    ...
    let source = Erc4626LiquiditySource { web3, settlement, registry };
    let init = move || {
    ...
    let collector = BackgroundInitLiquiditySource::new(
        "erc4626",
        init,
...
pub fn to_domain(id: liquidity::Id, order: Erc4626Order) -> Result<liquidity::Liquidity> {
    // At this stage, amounts are populated during route realization; here we only carry tokens and handler wiring
    ...
    Ok(liquidity::Liquidity {
        ...
        kind: liquidity::Kind::Erc4626(liquidity::erc4626::Edge {
            tokens: (
                eth::TokenAddress(a.into()),
                eth::TokenAddress(b.into()),
            ),
        }),
    })
}
...
pub fn to_wrap_interaction(
    ...
    // Note: _input is intentionally not used here; it's used for bounded approval generation elsewhere.
    tracing::debug!(
...
mod tests {
    ...
    fn encode_wrap_and_unwrap() {
        let input = liquidity::MaxInput(eth::Asset { token: eth::H160::zero().into(), amount: 123.into() });
        let output = liquidity::ExactOutput(eth::Asset { token: eth::H160::repeat_byte(0x11).into(), amount: 456.into() });
