    ...
    match liquidity.liquidity.kind.clone() {
        ...
        liquidity::Kind::Erc4626(edge) => {
            ...
            if edge.asset == sell && edge.vault == buy {
                // Wrap: asset -> vault (mint shares_out to settlement)
                ...
                // Unwrap: vault -> asset (withdraw assets_out to settlement)
...
mod test {
    ...
    fn erc4626_wrap_emits_bounded_approval_before_mint() {
        ...
        let settlement = eth::H160::from_low_u64_be(3);
        ...
        assert_eq!(allowances[0].0.amount, alloy::primitives::U256::from(100));
