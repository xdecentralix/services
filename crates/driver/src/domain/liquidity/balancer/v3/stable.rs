...
/// Liquidity data tied to a Balancer V3 stable pool.
///
/// These pools are an implementation of Curve.fi StableSwap pools [^1] on the
/// Balancer V3 Vault contract [^2]. Unlike V2, V3 uses a Batch Router for swaps
/// instead of direct Vault interaction.
///
...
impl Pool {
    ...
    pub fn swap(
        ...
        Ok(boundary::liquidity::balancer::v3::stable::to_interaction(
            self, input, output, receiver,
        ))
    }
}

/// Balancer V3 stable pool reserves.
///
...
/// Balancer V3 stable pool reserve for a single token.
...
pub enum InvalidAmplificationParameter {
    #[error("invalid Balancer V3 amplification parameter; 0 denominator")]
    ZeroDenominator,

    #[error("invalid Balancer V3 amplification parameter; overflow U256 representation")]
    Overflow,
}
