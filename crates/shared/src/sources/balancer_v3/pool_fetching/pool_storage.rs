//! Pool Storage contains all the essential models required by the balancer V3
...
/// PoolStorage represents in-memory storage of all deployed Balancer V3 Pools
    ...
    Factory: FactoryIndexing,
{
    ...
    pools_by_token: HashMap<H160, HashSet<H160>>,
    /// All indexed pool infos by ID.
    pools: HashMap<H160, Factory::PoolInfo>,
    ...
    Factory: FactoryIndexing,
{
    ...
    /// one of the pairs.
    pub fn pool_ids_for_token_pairs(&self, token_pairs: &HashSet<TokenPair>) -> HashSet<H160> {
        token_pairs
            .iter()
    ...
    pub fn pool_by_id(&self, pool_id: H160) -> Option<&Factory::PoolInfo> {
        self.pools.get(&pool_id)
    }

    /// Returns all pool infos by their IDs.
    pub fn pools_by_id(&self, pool_ids: &HashSet<H160>) -> Vec<Factory::PoolInfo> {
        pool_ids
            .iter()
...
mod tests {
    ...
    fn pool_init_data(start: usize, end: usize) -> PoolInitData {
        let pool_ids: Vec<H160> = (start..=end)
    ...
    fn initialize_storage() {
        ...
        assert_eq!(
            ...
            hashmap! {
                H160([0x11; 20]) => hashset![H160([1; 20]), H160([2; 20]), H160([3; 20])],
                H160([0x22; 20]) => hashset![H160([1; 20])],
                H160([0x33; 20]) => hashset![H160([2; 20])],
                H160([0x77; 20]) => hashset![H160([2; 20]), H160([3; 20])],
            }
        );
    }
    ...
    async fn insert_pool_events() {
        ...
        for (pool_created, block_created) in creation_events.iter().take(n) {
            pool_store
                .index_pool_creation(pool_created.clone(), *block_created)
                .await
                .unwrap();
        }
        ...
        for i in 0..n {
            weighted_pools.push(weighted::PoolInfo {
                common: common::PoolInfo {
                    ...
                    scaling_factors: vec![],
                    rate_providers: vec![H160::zero(); tokens[i..n].len()],
                    block_created: 0,
                    address: pool_addresses[i],
                },
                weights: vec![],
