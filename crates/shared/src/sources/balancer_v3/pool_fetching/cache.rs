...
use {
    ...
    crate::{
        ...
        sources::balancer_v3::pools::Pool,
    },
    anyhow::Result,
    ethcontract::H160,
...
type PoolCache<Inner> = RecentBlockCache<H160, Pool, CacheFetcher<Inner>>;
    ...
    Inner: InternalPoolFetching,
{
    pub fn new(
        ...
        let cache = RecentBlockCache::new(config, fetcher, block_stream, "balancerv3")?;
        Ok(Self { inner, cache })
    }
}
...
#[async_trait::async_trait]
impl<Inner> CacheFetching<H160, Pool> for CacheFetcher<Inner>
where
    Inner: InternalPoolFetching,
{
